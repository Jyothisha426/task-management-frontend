        ...
        {tasks.map((task, index) => (
          ...
          >
            <h3 className={styles.taskTitle}>
              <span className={`${styles.priorityIndicator} ${styles[`priority${task.priority}`]}`}></span>
              {task.title}
            </h3>
            ...
            {task.dueDate && (
              <p className={styles.taskDueDate}>Due: {new Date(task.dueDate).toLocaleDateString()}</p>
            )}
          </div>
        ))}
      </div>
